...
const mapDispatchToProps = dispatch => {
  return {
    onSearch: (event, query, option, page) => {
      event.preventDefault();
      dispatch(searchActions.requestSearch(query, option, page));
    },
    onInputChange: input => {
      dispatch(searchActions.searchQueryInput(input));
    },
    onSelectOption: option => {
      dispatch(searchActions.selectOption(option));
    }
  };
};
