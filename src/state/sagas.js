...
import {
  fetchTrendingMoviesSaga,
  fetchMovieDetailsSaga,
  fetchMovieCastSaga,
  fetchTopMoviesSaga,
  fetchPlayingMoviesSaga
} from "./movies/sagas";
...
import {
  fetchPopularPeopleSaga,
  fetchPeopleDetailsSaga,
  fetchPeopleCreditsSaga
} from "./people/saga";

function* watchMovies() {
  yield all([
    takeLatest(
      movieActionTypes.REQUEST_FETCH_TRENDING_MOVIES,
      fetchTrendingMoviesSaga
    ),
    takeLatest(
      movieActionTypes.REQUEST_FETCH_MOVIE_DETAILS,
      fetchMovieDetailsSaga
    ),
    ...
    takeLatest(
      movieActionTypes.REQUEST_FETCH_PLAYING_MOVIES,
      fetchPlayingMoviesSaga
    )
  ]);
}
...
function* watchSearch() {
  yield all([takeLatest(searchActionTypes.REQUEST_SEARCH, searchSaga)]);
}

function* watchPeople() {
  yield all([
    takeLatest(
      peopleActionTypes.REQUEST_FETCH_POPULAR_PEOPLE,
      fetchPopularPeopleSaga
    ),
    takeLatest(
      peopleActionTypes.REQUEST_FETCH_PEOPLE_DETAILS,
      fetchPeopleDetailsSaga
    ),
    takeLatest(
      peopleActionTypes.REQUEST_FETCH_PEOPLE_CREDITS,
      fetchPeopleCreditsSaga
    )
  ]);
}
...
export default function* rootSaga() {
  yield all([watchMovies(), watchSearch(), watchTv(), watchPeople()]);
}
